...
string ListaListas<T, Y>::SearchElemR(string el, NodoP<T, Y>* aux)
{
	...
	else
	{
		...
		{
			num = aux->getlist()->Search_DateR(el,pri,1);
			res = to_string(num);
		}
		else
		{
...
template<class T, class Y>
void ListaListas<T, Y>::ControlTime(string arc)
{
	timer t1;
	t1.start();
	SaveArchivo(arc);
	t1.stop();
	cout << "Tiempo Transcurrido: " << t1;
}

int main()
{
	ListaListas<int, string> list;
	list.ControlTime("cien anos de soledad.txt");
	cout << endl;
	list.show();
	return 0;
}
