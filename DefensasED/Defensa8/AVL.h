...
public:
    ...
    bool controlalturas(Nodo<T>* aux1, Nodo<T>* aux2);
    bool ControlAlturas(Nodo<T>* raiz);
};
...
{
    //cout << "Entro a la izquierda com" << endl;
...
{
    //cout << "Entro a la derecha com" << endl;
...
template<class T>
bool AVL<T>::controlalturas(Nodo<T>* aux1, Nodo<T>* aux2)
{
    bool res = true;
    int res1, res2, res3;
    res1 = Altura(aux1);
...
template<class T>
bool AVL<T>::ControlAlturas(Nodo<T>* raiz)
{
    bool res = true;
    if (raiz != NULL)
    {
        ...
        bool resb = true;
        int res1, res2, res3;
        resa = ControlAlturas(raiz->getLeft()); 
        resb = ControlAlturas(raiz->getRight()); 
        res1 = Altura(raiz->getLeft());
        res2 = Altura(raiz->getRight());
        res3 = res1 - res2;
        //control de alturas mas pequeñas
        if (!resa || !resb)
        {
            res = false;
        }
        //control de la altura actual
        if (res3 < -1 || res3>1)
        {
            res = false;
            cout << res3<<endl;
