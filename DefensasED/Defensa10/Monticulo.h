...
template<class T>
Nodo<T>** Monticulo<T>::sort(int &last1)
{
	Nodo<T>** vec = new Nodo<T>*[TAM]; 
	string palabra;
	while (last != 0)
	{
		palabra = ELIMINARS();
		vec[last1] = new Nodo<T>();
		vec[last1]->setDato(palabra);
		vec[last1]->setExiste(true);
		last1++;
	}
	return vec; 
}
